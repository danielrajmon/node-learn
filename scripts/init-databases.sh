...
docker exec -i -e PGPASSWORD="$POSTGRES_PASSWORD" $CONTAINER psql -U "$POSTGRES_USER" -d admin < "$(dirname "$0")/sql/events-ddl.sql"
echo "Applying DDL: events-views.sql -> admin DB"
docker exec -i -e PGPASSWORD="$POSTGRES_PASSWORD" $CONTAINER psql -U "$POSTGRES_USER" -d admin < "$(dirname "$0")/sql/events-views.sql"
